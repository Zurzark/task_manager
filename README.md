...
### 4. 📊 多维视图管理
- **列表视图 (List)**: 经典的树形结构，清晰展示父子任务关系，支持批量操作与全选。
- **看板视图 (Kanban)**: 基于优先级的流转管理 (重要紧急四分类)。
- **日历视图 (Calendar)**: 支持月/周视图，直观展示任务截止日期分布。
- **四象限视图 (Quadrant)**: 基于艾森豪威尔矩阵，聚焦 "重要且紧急" (Q1) 事务。
...
│   ├── store.js                # [数据] 核心状态管理 (Store)，负责 CRUD、筛选逻辑与持久化
